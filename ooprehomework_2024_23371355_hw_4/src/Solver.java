...
public class Solver {
    ...
    private void addBook() {
        String name = scanner.next();//nextLine()->next()
    ...
    private Book getBook(String name) {
        for (Book book : books) {
            if (book.getName().equals(name)) {
                //the comparison of the address is changed to the comparison between value
                return book;
            }
        }
        return null;
    ...
    private void cloneBookShelf() {
        ...
        Bookshelf newBookShelf = bookShelves.get(id1).cloneBookshelf();
        newBookShelf.setId(id2);
        bookShelves.put(id2, newBookShelf); //change the clone
    }
