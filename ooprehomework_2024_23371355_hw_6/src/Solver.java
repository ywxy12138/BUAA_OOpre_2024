...
public class Solver {
    ...
    public static void operateOp(ArrayList<String> in) {
        ...
        switch (in.get(0)) {
            ...
            case "2": {
                Bottle bo = opType(in.get(2),in.get(3),in.get(5),in.get(4),in.get(6));
                operationTwo(bo,markedAd);
                break;
            }
            case "3": {
                Equipment e = opEquip(in.get(2),in.get(3),in.get(5),in.get(4),in.get(6));
                operationThree(e,markedAd); //为该冒险者添加一个装备
                break;
            }
            case "4": {
            ...
            case "9": {
                String fraName = in.get(2);
                int welfareId = Integer.parseInt(in.get(3));
                opFrag(fraName, welfareId, markedAd);
                break;
            }
            case "10": {
    ...
    public static void operationTen(ArrayList<String> in, Adventurer markedAd) {
        ...
        int k = Integer.parseInt(in.get(3));
        ArrayList<Adventurer> list = new ArrayList<>();
        int maxDef = 0;
        for (int i = 0; i < k; ++i) {
            Adventurer ad = Solver.getAdventurers().get(Integer.parseInt(in.get(i + 4)));
            list.add(ad);
            if (ad.getDef() > maxDef) {
                maxDef = ad.getDef();
            }
        ...
        if (equipment != null && markedAd.getAtk() + equipment.getCE() > maxDef) {
            markedAd.success(equipment,list,markedAd);
        }
        else {
